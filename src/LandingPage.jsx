...
const LandingPage = ({ onGetStarted, onLogin }) => {
  ...
  return (
    <div style={styles.page}>
      <style>{keyframes + loaderCSS + extraUI}</style>
      ...
      {/* HEADER */}
      <header style={{ ...styles.header, animation: 'fadeDown .9s ease' }}>
      ...
      <main style={styles.heroWrap}>
        <div style={styles.heroText}>
          ...
          <div style={styles.actions}>
            <button className="btnGlow" style={styles.primaryBtnLg} onClick={onGetStarted}>
              Get Started <ArrowRight size={18} />
            </button>
            <button className="btnOutline" style={styles.secondaryBtn} onClick={onLogin}>
              View Demo
            </button>
          </div>
        </div>
      ...
      <section style={styles.features}>
        {[
          ...
          { icon: <BarChart3 />, title: 'Analytics', desc: 'Track conversion insights' }
        ].map((f, i) => (
          <div key={i} className="featureCard" style={styles.card}>
...
const styles = {
  ...
  badge: { padding: '6px 16px', borderRadius: 999, background: '#0f1b3d', color: '#60a5fa', display: 'inline-block', marginBottom: 16 },
  title: { fontSize: 52, lineHeight: 1.1, marginBottom: 18 },
  ...
  primaryBtnLg: { display: 'flex', gap: 8, alignItems: 'center', padding: '14px 22px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#3b82f6,#22d3ee)', color: '#fff', cursor: 'pointer' },
  secondaryBtn: { background: 'transparent', border: '1px solid #334155', color: '#cbd5e1', padding: '14px 22px', borderRadius: 12, cursor: 'pointer' },

  visual: { position: 'relative', height: 360 },
  glow: { position: 'absolute', width: 220, height: 220, borderRadius: '50%', filter: 'blur(90px)', opacity: 0.7, top: '20%', left: '30%', background: '#22d3ee' },

  features: { display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 30, padding: '60px' },
  card: { background: 'rgba(255,255,255,0.05)', padding: 30, borderRadius: 22 },
  cardIcon: { color: '#60a5fa', marginBottom: 16 },

  loaderOverlay: { position: 'fixed', inset: 0, background: '#050b1a', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }
};
...
const loaderCSS = `
.loader{
--main-size:4em;--text-color:#fff;--shine-color:#ffffff40;--shadow-color:#aaa;
display:flex;justify-content:center;align-items:center;
position:relative;font-size:var(--main-size);font-weight:900;
width:7.3em;height:1em;text-transform:uppercase;
}
.loader .text{position:absolute}
.loader .text span{
animation:scrolling 2s linear infinite;
background:linear-gradient(to right,var(--text-color),var(--shadow-color));
background-size:200%;
...
const extraUI = `
.btnGlow:hover{transform:translateY(-2px);box-shadow:0 0 25px #38bdf8}
.btnOutline:hover{background:#020617}
.featureCard{transition:.3s}
.featureCard:hover{transform:translateY(-10px);box-shadow:0 20px 50px #020617}
`;
